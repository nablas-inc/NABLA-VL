...
class SiglipVisionEmbeddings(nn.Module):
    ...
    def forward(
        ...
        for batch_idx, p_attn_mask in enumerate(patch_attention_mask):
            nb_patches_h = p_attn_mask.size(0)
            nb_patches_w = p_attn_mask.size(1)
            ...
            keep = p_attn_mask.view(-1).cpu()
            position_ids[batch_idx][keep] = pos_ids[keep]

            # pos_ids = torch.arange(0, max_nb_patches_h * max_nb_patches_w, 1, dtype=int)
            # position_ids[batch_idx][p_attn_mask.view(-1).cpu()] = pos_ids[p_attn_mask.view(-1).cpu()]
